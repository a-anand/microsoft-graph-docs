...
Change notifications enable you to subscribe to changes (create, update, and delete) to [membership](/graph/api/resources/chatMessage?preserve-view=true) in a [team](/graph/api/resources/team?preserve-view=true). Change notifications provide a low latency model by allowing you to maintain a [subscription](/graph/api/resources/webhooks?preserve-view=true). You can also get the resource data in the notifications and therefore avoid calling the API to get the payload.
